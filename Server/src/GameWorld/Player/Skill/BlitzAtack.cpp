...
void BlitzAtack::applyStep(std::map<uint16_t, std::shared_ptr<Infected>> &infecteds,
...
void BlitzAtack::throwBlitz(const std::pair<int16_t, int16_t>& pos) {
    if (this->available) {
        this->position = pos;
...
void BlitzAtack::blitz(std::map<uint16_t, std::shared_ptr<Infected>> &infecteds,
                       const double& stepTime) {
