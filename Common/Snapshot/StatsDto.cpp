...
StatsDto::StatsDto(const uint16_t& playerId, const int16_t& kills, const int32_t& rankingKills, const int16_t& shots,
        const int32_t& rankingShots, const int16_t& minutes, const int16_t& seconds, const int32_t& rankingDuration) : 
    playerId(playerId), kills(kills), rankingKills(rankingKills),
    shots(shots), rankingShots(rankingShots), minutes(minutes), seconds(seconds), rankingDuration(rankingDuration) {}

StatsDto::StatsDto(const uint16_t& playerId, const int16_t& kills, const int16_t& shots) : playerId(playerId),
    kills(kills), rankingKills(0), shots(shots), rankingShots(0), minutes(0), seconds(0), rankingDuration(0) {}

uint16_t StatsDto::getPlayerId() const {
    return playerId;
}
